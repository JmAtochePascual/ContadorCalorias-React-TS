...
export const activityReducer = (state: TActivityState = initialState, action: ActivitydAction) => {
  ...
  if (action.type === 'add-Activity') return { ...state, activities: [...state.activities, action.payload] };

  if (action.type === 'delete-Activity') return { ...state, activities: state.activities.filter(activity => activity.id !== action.payload) };

  if (action.type === 'set-id') return { ...state, activeId: action.payload };

  if (action.type === 'reset-Activity') return { activities: [], activeId: '' };
